...
import skmultiflow.trees as skt
...
ht = skt.HoeffdingTreeClassifier()
...
def prediction_user_type(level, keyX_pressed_count, keyY_pressed_count, respawn_enemy_count, respawn_coin_count):
	...
	return LABELS.get(y)
def pred_ut2(level, keyX_pressed_count, keyY_pressed_count, respawn_enemy_count, respawn_coin_count):
	global A0, A1
...
def show_game_over(screen_sizeX, screen_sizeY, score, high_score, coin_count, user_type,user_type2): 
	...
	message_display_center(user_type2, font_medium, yellow, int(screen_sizeX/2), int(screen_sizeY * 5/10))
...
PLAYER_NAME	= 'Phoomiphat'		#sws.PLAYER_NAME	# 'DAN'
...
#			score += level_score_increase
		...
		if game_over:
			...
			user_type2 = pred_ut2(level, keyX_pressed_count, keyY_pressed_count, respawn_enemy_count, respawn_coin_count)
			...
			for i in range(num_of_enemies):
				...
				if enemy[i].explosion_counter >= 1:
					...
					if is_collision(enemy[i], player):
						...
						X_scale = dt_scaler.transform(X)
						y1 = decision_tree.predict(X_scale)[0]
						y = ht.predict(X_scale)[0]
						X = np.array(X)
						yx = [y1]
						y1 = np.array(yx)
